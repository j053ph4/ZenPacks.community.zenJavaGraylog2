...
(function() {
            ...
            Ext.ComponentMgr.onAvailable('component-add-menu', function(config) {
                ...
                menuButton.menuItems.push({
                    xtype: 'menuitem',
                    text: _t('Add Graylog2 Metric') + '...',
                    hidden: Zenoss.Security.doesNotHavePermission('Manage Device'),
                    handler: function() {
                        var win = new Zenoss.dialog.CloseDialog({
                            width: 300,
                            title: _t('Add Graylog2 Metric'),
                            items: [{
                                xtype: 'form',
                                buttonAlign: 'left',
                                monitorValid: true,
                                ...
                                items:                         [
                            {
                                ...
                                buttons: [{
                                    xtype: 'DialogButton',
                                    id: 'zenJavaGraylog2-submit',
                                    text: _t('Add'),
                                    formBind: true,
                                    handler: function(b) {
                                        var form = b.ownerCt.ownerCt.getForm();
                                        var opts = form.getFieldValues();
                                        Zenoss.remote.zenJavaGraylog2Router.addGraylog2MetricRouter(opts,
                                        function(response) {
                                            if (response.success) {
                                                new Zenoss.dialog.SimpleMessageDialog({
                                                    title: _t('Graylog2 Metric Added'),
